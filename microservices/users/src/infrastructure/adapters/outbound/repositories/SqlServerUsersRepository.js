...
const ClienteMapper = require('../../../mappers/ClienteMapper');
const AdministradorMapper = require('../../../mappers/AdministradorMapper');
const { getPool, sql } = require('../../../../../../../database/sqlServerConfig');
const IUsersRepositoryPort = require('../../../../domain/ports/outbound/IUsersRepositoryPort');

class SqlServerUsersRepository extends IUsersRepositoryPort {
  async createCliente(data) {
    try {
      ...
      return ClienteMapper.toDomain({
        id: dataCliente.id,
        created_at: dataCliente.created_at,
  ...
  async findAllClientes() {
    try {
      ...
      return result.recordset.map(data => ClienteMapper.toDomain(data));
    } catch (error) {
  ...
  async createAdministrador(adminData) {
    try {
      ...
      return AdministradorMapper.toDomain({
        id: data.id,
  ...
  async findAdministradorByUsuarioId(usuarioId) {
    try {
      ...
      const data = result.recordset[0];
      return AdministradorMapper.toDomain(data);
    } catch (error) {
  ...
  async findAllAdministradores() {
    try {
      ...
      return result.recordset.map(data => AdministradorMapper.toDomain(data));
    } catch (error) {
  ...
  async findAllUsuarios() {
    try {
      ...
      `);
      return result.recordset.map(data => UserMapper.toDomain(data));
    } catch (error) {
  ...
  async toggleUsuarioActivo(usuarioId) {
    try {
      ...
      const result = await pool
        ...
        .query('UPDATE usuarios SET activo = @activo, updated_at = GETDATE() OUTPUT INSERTED.* WHERE id = @id');
      return UserMapper.toDomain(result.recordset[0]);
    } catch (error) {
