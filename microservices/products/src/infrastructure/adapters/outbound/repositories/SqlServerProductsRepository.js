...
class SqlServerProductsRepository extends IProductsRepositoryPort {
  async createProducto(productoData) {
    try {
      ...
      return ProductoMapper.toDomain(result.recordset[0]);
    } catch (error) {
  ...
  async findAllProductos(filters = {}) {
    try {
      ...
      const result = await request.query(query);
      return result.recordset.map(item => ProductoMapper.toDomain(item));
    } catch (error) {
