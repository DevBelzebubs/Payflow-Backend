...
class OrdersService {
  ...
  async createOrden(ordenData, datosPago) {
    const productsBaseUrl = await resolveService("products-service");
    const serviceBaseUrl = await resolveService("services-service");
    const bankAccountBaseUrl = await resolveService("bank-accounts-service");
    const { clienteId, items, notas } = ordenData;
    if (!clienteId)
      throw new Error("El clienteId es requerido para crear la orden.");

    console.log(
      `[OrdersService] Iniciando creación de orden para cliente ${clienteId}`
    );
      ...
      for (const item of items) {
        ...
        if (item.productoId) {
          const p = await axios.get(
            `${productsBaseUrl}/api/productos/${item.productoId}`
          );
          ...
          if (item.ticketTypeId) {
            const t = await axios.get(
              `${serviceBaseUrl}/api/servicios/${item.servicioId}/tipos-entrada`
            );
            ...
            const s = await axios.get(
              `${serviceBaseUrl}/api/servicios/${item.servicioId}`
            );
    ...
    if (datosPago.origen === "MERCADOPAGO") {
      ...
      const mpBody = {
        ...
        back_urls: {
          success:
            "http://localhost:3010/dashboard/history?status=success&type=cart",
          failure:
            "http://localhost:3010/dashboard/payment/checkout?status=failure",
          pending:
            "http://localhost:3010/dashboard/payment/checkout?status=pending",
        },
      ...
      try {
        ...
        throw new Error(
          `Fallo al crear preferencia en Mercado Pago: ${mpError.message}`
        );
      }
    }
    ...
    if (datosPago.origen === "BCP") {
      ...
      const bcpRes = await this.sendBcpRequestWithRetry({
        method: "post",
        url,
        data: debitoRequest,
      });
      ...
      console.log(
        `[OrdersService] Iniciando débito interno Payflow. Cuenta: ${datosPago.cuentaId}, Monto: ${total}`
      );
      await axios.post(
        `${bankAccountBaseUrl}/api/cuentas-bancarias/debitar`,
        {
    ...
    console.log(
      `[OrdersService] Orden ${orden.id} creada exitosamente. Procesando items...`
    );
    ...
    for (const item of items) {
      ...
      if (item.productoId) {
        try {
          console.log(
            `[OrdersService] 📉 Descontando stock para producto ${item.productoId}. Cantidad a restar: ${item.cantidad}`
          );
          const stockUrl = `${productsBaseUrl}/api/productos/${item.productoId}/stock`;
          ...
          if (orden.items && orden.items.length > 0) {
            for (const item of orden.items) {
              if (item.producto_id) {
                try {
                  await axios.patch(
                    `${productsBaseUrl}/api/productos/${item.producto_id}/stock`,
                    {
                      cantidad: -item.cantidad,
                    }
                  );
  ...
  async procesarRenovaciones() {
    ...
    for (const sub of suscripcionesVencidas) {
      try {
        ...
        console.error(
          `[Renovación] Fallo al renovar suscripción ${sub.id}:`,
          error.message
        );
        resultados.fallidos++;
      }
